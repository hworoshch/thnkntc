...
class Main
  ...
  def trains_menu
    loop do
      ...
      when 4 then create_car
      when 5 then show_train_cars
      when 6 then load_car
      when 0 then break
      end
    end
  end
  ...
  def train_add_car(train)
    case train.type
    when 'cargo' then train.add_car(CargoCar.all_detached.first)
    when 'passenger' then train.add_car(PassengerCar.all_detached.first)
    end
  end
  ...
  def create_car
    ...
    show_menu(TRAINS_CREATE_MENU)
    case gets.to_i
    when 1 then CargoCar.new(capacity)
    when 2 then PassengerCar.new(capacity)
    when 0 then return
    end
    puts "Вагон создан"
  end
